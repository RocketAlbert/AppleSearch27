...
class AppleItemController {
    ...
    static func fetchAppleItemFor(term: String, completion: @escaping([AppleItem]?) -> Void) {
        ...
        let searchTermQuey = URLQueryItem(name: "term", value: term)
        // term\(term)
        // term = nickelback
        let mediaQuery = URLQueryItem(name: "media", value: "music")
        // media = music
        
        // At the end of these QueryItems
        // search?term=\(term)&media=music
        
        components?.queryItems = [searchTermQuey, mediaQuery]
        guard let finalURL = components?.url else {completion(nil); return}
        
        URLSession.shared.dataTask(with: finalURL) { (data, _, error) in
            if let error = error {
                print("Oh snap, that seach didn't work: \(error.localizedDescription)")
                completion(nil)
                return
            }
